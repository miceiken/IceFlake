    ...
    public static class WoWWorld
    {
        ...
        public static bool HandleTerrainClick(Location loc)
        {
            if (_handleTerrainClick == null)
                _handleTerrainClick =
                    Manager.Memory.RegisterDelegate<HandleTerrainClickDelegate>(
                        (IntPtr) Pointers.World.HandleTerrainClick, true);

            return
                _handleTerrainClick(new TerrainClickEvent
                {
                    GUID = 0ul,
                    Position = loc,
                    Button = MouseButton.None | MouseButton.Left
                });
        }

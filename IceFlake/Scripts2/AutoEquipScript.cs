    ...
    public class AutoEquipScript : Script
    {
        ...
        public override void OnTick()
        {
            ...
            foreach (var item in items.Where(x => !_blacklistedItems.Contains(x.Guid)))
            {
                ...
                {
                    var currentItem = Manager.LocalPlayer.GetEquippedItem(slot) ?? WoWItem.Invalid;
                    if (!currentItem.IsValid)
                    {
                        if (slot == EquipSlot.OffHand)
                        {
                            var mh = Manager.LocalPlayer.GetEquippedItem(EquipSlot.MainHand) ?? WoWItem.Invalid;
                    ...
                    if (item.ItemInfo.Class == ItemClass.Weapon)
                    {
                            ...
                            && currentItem.ItemSparseInfo.InventoryType == InventoryType.WeaponMainHand)
                        {
                            ...
                            var offHand = Manager.LocalPlayer.GetEquippedItem(EquipSlot.OffHand) ?? WoWItem.Invalid;
                            if (offHand.IsValid)
        ...
        private void EvaluateRollItem(int rollId)
        {
            ...
            var itemSuffix = Convert.ToUInt32(splitItemString.ElementAtOrDefault(7));
            if (itemSuffix > 0)
            {
                var suffix = WoWDB.GetTable(ClientDB.ItemRandomProperties).GetRow(itemSuffix).GetField<string>(7);
        ...
        private int CalculateItemScore(ItemSparseRec itemSparseRec, ItemRec itemRec)
        {
            var score = 0;
            ...
            if (itemRec.Class == ItemClass.Armor)
            {
                if (_weightSet.ContainsKey(ItemStatType.Armor))
                    score += (int)Math.Floor(itemSparseRec.Armor * _weightSet[ItemStatType.Armor]);
                else
                    score += (int)itemSparseRec.Armor;
            }
            if (itemRec.Class == ItemClass.Weapon)
            {
                if (_weightSet.ContainsKey(ItemStatType.DPS))
                    score += (int)Math.Floor(itemSparseRec.DPS * _weightSet[ItemStatType.DPS]);
                else
                    score += (int)itemSparseRec.DPS;
            }

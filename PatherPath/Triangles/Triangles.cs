...
namespace WowTriangles
{
	...
	public class ChunkedTriangleCollection
	{
                ...
                if (Utils.abs(normal.z) > angle_z || IgnoreGradient)
                {
                    ...
                    if (hasIntersected)
                    {
                        if (intersect.z > best_z)
                        {
                            ...
                            {
                                System.Diagnostics.Debug.WriteLine("new best z:" + intersect.z.ToString());
                                best_z = intersect.z;
                                best_flags = t_flags;
                                found = true;
                            }
                        ...
                        {
                            //System.Diagnostics.Debug.WriteLine("not best");
                        }
                    }
                    else
                    {
                        //System.Diagnostics.Debug.WriteLine("Utils.SegmentTriangleIntersect false");
                    }
                }
                else
                {
                    //System.Diagnostics.Debug.WriteLine(string.Format("utils.abs(normal.z) {0} > angle_z {1}", Utils.abs(normal.z), angle_z));
                }
            }
            if (found)
            {
        ...
        public bool LineOfSightExists(Pather.Graph.Spot a, Pather.Graph.Spot b)
        {

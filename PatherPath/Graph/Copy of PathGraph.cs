...
namespace Pather.Graph
{
	public class PathGraph
	{
		...
		*/
		...
		public string BaseDir = "Libs\\PathInfo";
		string Continent;
		...
		public Spot TryAddSpot(Spot wasAt, Location isAt)
		{
			...
			if (isAtSpot == null)
			{
                ...
                GContext.logSystem("Learned a new spot at " + isAtSpot.location + " connected to " + connected + " other spots");
				wasAt = isAtSpot;
			}
			else
			{
        ...
        public static Spot LastSearchSpot = null;
        ...
        private class SearchProgress
        {
            ...
            System.DateTime lastSpam = System.DateTime.Now;
            Spot src;
            Spot dst;
            bool isInCombat = GPlayerSelf.Me.IsInCombat;
            
            public SearchProgress(Spot src, Spot dst)
            {
                this.src = src;
                this.dst = dst;
            }

            public bool CheckProgress(Spot BestSpot)
            {
                if (count % 100 == 0)
                {
                    ...
                    if (span.Seconds != 0 && BestSpot != null)
                    {
                        ...
                        LogStatus( "searching.... ", (count - prevCount));

                        //check I did not enter combat during search
                        if (!isInCombat && GPlayerSelf.Me.IsInCombat)
                        {
                            Log("Entered combat during search");
                            return false;
                        }
                        ...
                        if (System.DateTime.Now.Subtract(pre).Seconds > 30)
                        {
            ...
            {
                LogStatus(result, count);
            }
            public void LogStatus(string result,int cnt)
            {
                System.TimeSpan ts = System.DateTime.Now.Subtract(pre);
                int t = ts.Seconds * 1000 + ts.Milliseconds;
                if (t == 0) { t = 1; }
                Log(result + (cnt * 1000) / t + " steps/s from " + src.ToString() + " to " + dst.ToString());
            }
                            ...
                            ILocationHeuristics locationHeuristics)
        {
            ...
            SearchProgress searchProgress = new SearchProgress(src, dst);
            ...
            {
                float prio;
                LastSearchSpot = q.Dequeue(out prio); // .Value; 

                if (LastSearchSpot.SearchIsClosed(currentSearchID)) { continue; }
                LastSearchSpot.SearchClose(currentSearchID);

                //quit if we have taken too long, or have gone into combat
                if (!searchProgress.CheckProgress(BestSpot)) { break; }
                
                // are we there?
                if (LastSearchSpot.Equals(dst) || LastSearchSpot.location.GetDistanceTo(realDst) <= minHowClose)
                {
                    return LastSearchSpot; // got there
                }

                if (BestSpot == null || LastSearchSpot.location.GetDistanceTo(realDst) < BestSpot.location.GetDistanceTo(realDst))
                {
                    BestSpot = LastSearchSpot;
                }

                
                List<Spot> ll = LastSearchSpot.GetPathsToSpots(this);
                foreach (Spot to in ll)
                {
                    if (to != null && !to.IsBlocked() && !to.SearchIsClosed(currentSearchID))
                    {
                        ScorePath(to, dst, currentSearchID, locationHeuristics, q);
                    }
                }

                //hmm search the triangles :p
                if (!LastSearchSpot.GetFlag(Spot.FLAG_MPQ_MAPPED))
                {
                    float PI = (float)Math.PI;

                    LastSearchSpot.SetFlag(Spot.FLAG_MPQ_MAPPED, true);
                    for (float a = 0; a < PI * 2; a += PI / 8)
                    {
                        float nx = LastSearchSpot.X + (float)Math.Sin(a) * WantedStepLength;// *0.8f;
                        float ny = LastSearchSpot.Y + (float)Math.Cos(a) * WantedStepLength;// *0.8f;

                        Spot s = GetSpot(nx, ny, LastSearchSpot.Z);

                        if (s == null)
                        {
                            s = FindClosestSpot(new Location(nx, ny, LastSearchSpot.Z), MinStepLength); // TODO: this is slow
                        }
                        ...
                        if (s == null)
                        {
                            ...
                            if (triangleWorld.FindStandableAt(nx, ny,LastSearchSpot.Z - WantedStepLength * .75f, LastSearchSpot.Z + WantedStepLength * .75f,out new_z, out flags, toonHeight, toonSize))
                            {
                                s = FindClosestSpot(new Location(nx, ny, new_z), MinStepLength);
                                if (s == null)
                                {
                                    if (!triangleWorld.IsStepBlocked(LastSearchSpot.X, LastSearchSpot.Y, LastSearchSpot.Z, nx, ny, new_z,toonHeight, toonSize, null))
                                    {
                                        Spot n = new Spot(nx, ny, new_z);
                                        if (triangleWorld.LineOfSightExists(LastSearchSpot, n))
                                        {
                                            ...
                                            //    if (LastSearchSpot.HasPathTo(to.location))
                                            //    {
                                            //        ScorePath(to, dst, currentSearchID, locationHeuristics, q);
                                            //    }
                                            //    else
        ...
        public void ScorePath(Spot to, Spot dst, int currentSearchID, ILocationHeuristics locationHeuristics, PriorityQueue<Spot, float> q)
        {
            float src_score = LastSearchSpot.SearchScoreGet(currentSearchID);

            float old_score = 1E30f;

            float new_score = src_score + LastSearchSpot.GetDistanceTo(to) + TurnCost(LastSearchSpot, to);
            if (locationHeuristics != null) { new_score += locationHeuristics.Score(LastSearchSpot.X, LastSearchSpot.Y, LastSearchSpot.Z); }
            if (to.GetFlag(Spot.FLAG_WATER)) { new_score += 30; }
            if (triangleWorld.IsCloseToModel(to.X, to.Y, to.Z, 2)) { new_score += 10; }
            new_score += triangleWorld.GradiantScore(to.X, to.Y, to.Z, 2);

            if (to.SearchScoreIsSet(currentSearchID)) { old_score = to.SearchScoreGet(currentSearchID); }

            if (new_score < old_score)
            {
                // shorter path to here found
                to.traceBack = LastSearchSpot;
        ...
        public static Path LastPath = null;

		public Path CreatePath(Spot from, Spot to, Location realDst,float minHowClose, bool AllowInvented,ILocationHeuristics locationHeuristics)
		{
		...
		public Path CreatePath(Location fromLoc, Location toLoc,float howClose)
		{
		...
		{
            GContext.Log("Creating Path from " + fromLoc.ToString() + " tot " + toLoc.ToString());
            ...
            PPather.WriteLine("CreatePath took {0}",-t.TicksLeft);
			if (rawPath == null)
			{
				return null;
			}
